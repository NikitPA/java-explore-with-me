...
create table compilation_event (compilation_id int4 not null, event_id int4 not null);
create table compilations (id  serial not null, pinned boolean not null, title varchar(255), primary key (id));
create table events (id  serial not null, annotation varchar(2000), confirmed_requests int4 not null, created_on timestamp, description varchar(7000), event_date timestamp, paid boolean not null, participant_limit int4 not null, published_on timestamp, request_moderation boolean not null, state varchar(255), title varchar(120), views int4 not null, category_id int4, initiator_id int4, location_id int4, primary key (id));
create table locations (id  serial not null, lat float4 not null, lon float4 not null, primary key (id));
create table requests (id  serial not null, created timestamp, status varchar(255), event_id int4, requester_id int4, primary key (id));
create table users (id  serial not null, email varchar(255), name varchar(255), primary key (id));
